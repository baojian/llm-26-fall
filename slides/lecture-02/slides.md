...
<!-- .slide: id="zero-probabilities" -->

## Motivation: zero probabilities

<p><strong>Maximum likelihood estimate (trigram):</strong> $q(w_i \mid w_{i-2}w_{i-1}) = \dfrac{C(w_{i-2}w_{i-1}w_i)}{C(w_{i-2}w_{i-1})}$. Given the following datasets:</p>

<div class="columns">
<div><p><strong class="text-good">Training set</strong></p><p class="text-token">... denied the allegations<br>... denied the reports<br>... denied the claims<br>... denied the request</p></div>
<div><p><strong class="text-good">Test set</strong></p><p class="text-bad">... denied the offer<br>... denied the loan<br><strong>$P(\text{offer} \mid \text{denied the})$ ?</strong></p></div>
</div>

<ul>
<li>Trigrams can have <strong>zero probability</strong> (unseen $n$-grams ⇒ assign 0).</li>
<li class="fragment"><strong>Question:</strong> how to avoid these zeros?</li>
</ul>

Note:
One zero factor makes the whole sentence probability zero and the perplexity infinite. Source: Spring Lecture 02 slide 20, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/19.

---

<!-- .slide: id="smoothing-intuition" -->

## Smoothing: intuition

- When we have **sparse statistics**: $q(w \mid \text{denied the})$
- **Steal probability mass** to generalize better.
- **How to steal?** Popular methods:
  - Additive smoothing
  - Linear interpolation
  - Backoff smoothing (Katz)
  - Good–Turing and Kneser–Ney smoothing

Note:
The next two charts show the raw counts after “denied the” and the counts after moving some mass to “other.” Source: Spring Lecture 02 slide 21, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/20.

---

<!-- .slide: id="smoothing-raw-counts" -->

## Raw counts after “denied the”

<div class="plot" data-plotly="assets/smoothing-raw-counts.json" role="img" aria-label="Raw counts of words following denied the: allegations 3, reports 2, claims 1, request 1, and zero for attack, man, and outcome."></div>

<p class="caption">Many zeros: every word not seen after “denied the” gets probability 0.</p>

Note:
Seven counts in total. The three zero bars stand for the whole rest of the vocabulary. Source: Spring Lecture 02 slide 21, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/20.

---

<!-- .slide: id="smoothing-after" -->

## After smoothing: some mass for “other”

<div class="plot" data-plotly="assets/smoothing-after.json" role="img" aria-label="Counts after smoothing: allegations 2.5, reports 1.5, claims 0.5, request 0.5, and 2 for other, shown in green."></div>

<p class="caption">$P(w \mid \text{denied the})$: 2.5 allegations · 1.5 reports · 0.5 claims · 0.5 request · <span class="text-good">2 other</span></p>

Note:
The total mass stays 7. Half a count is removed from each seen word and the two stolen counts are given to all unseen words together. Source: Spring Lecture 02 slide 21, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/20.

---

<!-- .slide: id="additive-smoothing" -->

## Method 1: Additive smoothing (Laplace / Add-$\delta$)

<p><strong>1) Add-one (Laplace) idea.</strong> MLE estimate:</p>
<p>$$P_{\text{MLE}}(w_i\mid w_{i-1}) =\frac{C(w_{i-1}w_i)}{C(w_{i-1})}$$</p>
<p><span class="text-good"><strong>Pretend we saw each word one more time:</strong></span></p>
<p>$$P_{\text{Add}}(w_i\mid w_{i-1}) = \frac{C(w_{i-1}w_i)+1}{C(w_{i-1})+|V|}$$</p>

Note:
The denominator adds $|V|$ because every one of the $|V|$ possible next words received one extra count. Source: Spring Lecture 02 slide 22, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/21.

---

<!-- .slide: id="pseudo-counts" -->

## Additive smoothing as pseudo-counts $C^{\ast}$

<p><strong>2) View as pseudo-counts.</strong> “Reconstitute” <span class="text-good"><strong>pseudo-counts</strong></span> from smoothed probabilities.</p>
<p><strong>Unigrams</strong> (train size $N$):</p>
<p>$$C^{\ast}(w_i) = P_{\text{Add}}(w_i)\cdot N = \frac{C(w_i)+1}{N+|V|}\cdot N$$</p>
<p><strong>Bigrams:</strong></p>
<p>$$C^{\ast}(w_{i-1}w_i) = P_{\text{Add}}(w_i\mid w_{i-1})\cdot C(w_{i-1}) = \frac{C(w_{i-1}w_i)+1}{C(w_{i-1})+|V|}\cdot C(w_{i-1})$$</p>

Note:
Pseudo-counts let us compare the smoothed model with the raw counts on the same scale, which the example slides use. Source: Spring Lecture 02 slide 22, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/21.

---

<!-- .slide: id="add-delta" -->

## Generalization: Add-$\delta$ smoothing

<p><strong>3)</strong> Add-one is often too aggressive. Use <span class="text-bad"><strong>Add-$\delta$</strong></span> (typically $0 \lt \delta \le 1$):</p>
<p>$$P_{\text{Add}}(w_i\mid w_{i-1}) =\frac{C(w_{i-1}w_i)+\delta}{\sum_{w\in V}\big(C(w_{i-1}w)+\delta\big)} =\frac{C(w_{i-1}w_i)+\delta}{C(w_{i-1})+\delta|V|}$$</p>
<p>Still simple — better methods (backoff / interpolation) usually work better.</p>

Note:
$\delta$ is tuned on held-out data. Source: Spring Lecture 02 slide 22, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/21.

---

<!-- .slide: id="laplace-counts" -->

## Additive smoothing example: bigram counts $C(w_{i-1}w_i)$

<table class="compact">
<thead><tr><th></th><th class="num">i</th><th class="num">want</th><th class="num">to</th><th class="num">eat</th><th class="num">chinese</th><th class="num">food</th><th class="num">lunch</th><th class="num">spend</th></tr></thead>
<tbody>
<tr><th>i</th><td class="num">5</td><td class="num">827</td><td class="num">0</td><td class="num">9</td><td class="num">0</td><td class="num">0</td><td class="num">0</td><td class="num">2</td></tr>
<tr><th>want</th><td class="num">2</td><td class="num">0</td><td class="num">608</td><td class="num">1</td><td class="num">6</td><td class="num">6</td><td class="num">5</td><td class="num">1</td></tr>
<tr><th>to</th><td class="num">2</td><td class="num">0</td><td class="num">4</td><td class="num">686</td><td class="num">2</td><td class="num">0</td><td class="num">6</td><td class="num">211</td></tr>
<tr><th>eat</th><td class="num">0</td><td class="num">0</td><td class="num">2</td><td class="num">0</td><td class="num">16</td><td class="num">2</td><td class="num">42</td><td class="num">0</td></tr>
<tr><th>chinese</th><td class="num">1</td><td class="num">0</td><td class="num">0</td><td class="num">0</td><td class="num">0</td><td class="num">82</td><td class="num">1</td><td class="num">0</td></tr>
<tr><th>food</th><td class="num">15</td><td class="num">0</td><td class="num">15</td><td class="num">0</td><td class="num">1</td><td class="num">4</td><td class="num">0</td><td class="num">0</td></tr>
<tr><th>lunch</th><td class="num">2</td><td class="num">0</td><td class="num">0</td><td class="num">0</td><td class="num">0</td><td class="num">1</td><td class="num">0</td><td class="num">0</td></tr>
<tr><th>spend</th><td class="num">1</td><td class="num">0</td><td class="num">1</td><td class="num">0</td><td class="num">0</td><td class="num">0</td><td class="num">0</td><td class="num">0</td></tr>
</tbody>
</table>

<p class="caption">Rows: $w_{i-1}$. Columns: $w_i$. Berkeley Restaurant Project corpus, $|V|=1446$.</p>

Note:
Counts from Jurafsky and Martin, Chapter 3 (Berkeley Restaurant Project). Unigram counts used on the following slides: i 2533, want 927, to 2417, eat 746, chinese 158, food 1093, lunch 341, spend 278. Source: Spring Lecture 02 slide 23, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/22.

---

<!-- .slide: id="laplace-plus-one" -->

## Additive smoothing example: Laplace counts $C(w_{i-1}w_i)+1$

<table class="compact">
<thead><tr><th></th><th class="num">i</th><th class="num">want</th><th class="num">to</th><th class="num">eat</th><th class="num">chinese</th><th class="num">food</th><th class="num">lunch</th><th class="num">spend</th></tr></thead>
<tbody>
<tr><th>i</th><td class="num">6</td><td class="num">828</td><td class="num">1</td><td class="num">10</td><td class="num">1</td><td class="num">1</td><td class="num">1</td><td class="num">3</td></tr>
<tr><th>want</th><td class="num">3</td><td class="num">1</td><td class="num">609</td><td class="num">2</td><td class="num">7</td><td class="num">7</td><td class="num">6</td><td class="num">2</td></tr>
<tr><th>to</th><td class="num">3</td><td class="num">1</td><td class="num">5</td><td class="num">687</td><td class="num">3</td><td class="num">1</td><td class="num">7</td><td class="num">212</td></tr>
<tr><th>eat</th><td class="num">1</td><td class="num">1</td><td class="num">3</td><td class="num">1</td><td class="num">17</td><td class="num">3</td><td class="num">43</td><td class="num">1</td></tr>
<tr><th>chinese</th><td class="num">2</td><td class="num">1</td><td class="num">1</td><td class="num">1</td><td class="num">1</td><td class="num">83</td><td class="num">2</td><td class="num">1</td></tr>
<tr><th>food</th><td class="num">16</td><td class="num">1</td><td class="num">16</td><td class="num">1</td><td class="num">2</td><td class="num">5</td><td class="num">1</td><td class="num">1</td></tr>
<tr><th>lunch</th><td class="num">3</td><td class="num">1</td><td class="num">1</td><td class="num">1</td><td class="num">1</td><td class="num">2</td><td class="num">1</td><td class="num">1</td></tr>
<tr><th>spend</th><td class="num">2</td><td class="num">1</td><td class="num">2</td><td class="num">1</td><td class="num">1</td><td class="num">1</td><td class="num">1</td><td class="num">1</td></tr>
</tbody>
</table>

<p class="caption">Every cell gains one, including the 1446 columns not shown.</p>

Note:
Source: Spring Lecture 02 slide 23, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/22.

---

<!-- .slide: class="exercise" id="exercise-04" -->

## Using Laplace smoothing: probabilities

<p class="exercise-meta">Exercise E04 · 4 minutes · Notebook E04 · Compute $P_{\text{Lap}}(\text{want}\mid\text{i}) = \frac{C(\text{i want})+1}{C(\text{i})+|V|}$ with $C(\text{i})=2533$, $|V|=1446$</p>

<table class="compact">
<thead><tr><th></th><th class="num">i</th><th class="num">want</th><th class="num">to</th><th class="num">eat</th><th class="num">chinese</th><th class="num">food</th><th class="num">lunch</th><th class="num">spend</th></tr></thead>
<tbody>
<tr><th>i</th><td class="num">0.0015</td><td class="num"><span class="answer fragment">0.21</span></td><td class="num">0.00025</td><td class="num">0.0025</td><td class="num">0.00025</td><td class="num">0.00025</td><td class="num">0.00025</td><td class="num">0.00075</td></tr>
<tr><th>want</th><td class="num">0.0013</td><td class="num">0.00042</td><td class="num">0.26</td><td class="num">0.00084</td><td class="num">0.0029</td><td class="num">0.0029</td><td class="num">0.0025</td><td class="num">0.00084</td></tr>
<tr><th>to</th><td class="num">0.00078</td><td class="num">0.00026</td><td class="num">0.0013</td><td class="num">0.18</td><td class="num">0.00078</td><td class="num">0.00026</td><td class="num">0.0018</td><td class="num">0.055</td></tr>
<tr><th>eat</th><td class="num">0.00046</td><td class="num">0.00046</td><td class="num">0.0014</td><td class="num">0.00046</td><td class="num">0.0078</td><td class="num">0.0014</td><td class="num">0.02</td><td class="num">0.00046</td></tr>
<tr><th>chinese</th><td class="num">0.0012</td><td class="num">0.00062</td><td class="num">0.00062</td><td class="num">0.00062</td><td class="num">0.00062</td><td class="num">0.052</td><td class="num">0.0012</td><td class="num">0.00062</td></tr>
<tr><th>food</th><td class="num">0.0063</td><td class="num">0.00039</td><td class="num">0.0063</td><td class="num">0.00039</td><td class="num">0.00079</td><td class="num">0.002</td><td class="num">0.00039</td><td class="num">0.00039</td></tr>
<tr><th>lunch</th><td class="num">0.0017</td><td class="num">0.00056</td><td class="num">0.00056</td><td class="num">0.00056</td><td class="num">0.00056</td><td class="num">0.0011</td><td class="num">0.00056</td><td class="num">0.00056</td></tr>
<tr><th>spend</th><td class="num">0.0012</td><td class="num">0.00058</td><td class="num">0.0012</td><td class="num">0.00058</td><td class="num">0.00058</td><td class="num">0.00058</td><td class="num">0.00058</td><td class="num">0.00058</td></tr>
</tbody>
</table>

Note:
Expected: $(827+1)/(2533+1446)=828/3979=0.208$. The cell appears after the attempt. All 64 values were recomputed from the counts and unigram totals in Notebook E04; they match to two significant digits. Source: Spring Lecture 02 slide 23, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/22.

---

<!-- .slide: id="reconstituted-counts" -->
...
<thead><tr><th></th><th class="num">i</th><th class="num">want</th><th class="num">to</th><th class="num">eat</th><th class="num">chinese</th><th class="num">food</th><th class="num">lunch</th><th class="num">spend</th></tr></thead>
<tbody>
<tr><th>i</th><td class="num">3.8</td><td class="num"><strong class="text-bad">527</strong></td><td class="num">0.64</td><td class="num">6.4</td><td class="num">0.64</td><td class="num">0.64</td><td class="num">0.64</td><td class="num">1.9</td></tr>
<tr><th>want</th><td class="num">1.2</td><td class="num">0.39</td><td class="num"><strong class="text-bad">238</strong></td><td class="num">0.78</td><td class="num">2.7</td><td class="num">2.7</td><td class="num">2.3</td><td class="num">0.78</td></tr>
<tr><th>to</th><td class="num">1.9</td><td class="num">0.63</td><td class="num">3.1</td><td class="num">430</td><td class="num">1.9</td><td class="num">0.63</td><td class="num">4.4</td><td class="num">133</td></tr>
<tr><th>eat</th><td class="num">0.34</td><td class="num">0.34</td><td class="num">1</td><td class="num">0.34</td><td class="num">5.8</td><td class="num">1</td><td class="num">15</td><td class="num">0.34</td></tr>
<tr><th>chinese</th><td class="num">0.2</td><td class="num">0.098</td><td class="num">0.098</td><td class="num">0.098</td><td class="num">0.098</td><td class="num">8.2</td><td class="num">0.2</td><td class="num">0.098</td></tr>
<tr><th>food</th><td class="num">6.9</td><td class="num">0.43</td><td class="num">6.9</td><td class="num">0.43</td><td class="num">0.86</td><td class="num">2.2</td><td class="num">0.43</td><td class="num">0.43</td></tr>
<tr><th>lunch</th><td class="num">0.57</td><td class="num">0.19</td><td class="num">0.19</td><td class="num">0.19</td><td class="num">0.19</td><td class="num">0.38</td><td class="num">0.19</td><td class="num">0.19</td></tr>
<tr><th>spend</th><td class="num">0.32</td><td class="num">0.16</td><td class="num">0.32</td><td class="num">0.16</td><td class="num">0.16</td><td class="num">0.16</td><td class="num">0.16</td><td class="num">0.16</td></tr>
</tbody>
</table>

<p class="caption"><strong class="text-token">Some counts changed too much!</strong> Before: $C(\text{i want})=827$, $C(\text{want to})=608$.</p>

Note:
Compare with the raw count table: 827 became 527 and 608 became 238. Add-one moves far too much mass to the unseen events when $|V|$ is large relative to $C(w_{i-1})$. Source: Spring Lecture 02 slide 24, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/23.
...
<p><strong>Bigram case</strong></p>
<ul>
<li>If some bigrams are unseen (e.g., $C(\text{burnish the})=0$), MLE gives zero: $P(\text{the}\mid \text{burnish})=0$.</li>
<li>Combine higher-order and lower-order information via <strong>linear interpolation</strong> (Mercer, 1980):</li>
</ul>
<p>$$P_{\text{Int}}(w_n\mid w_{n-1}) = \lambda\cdot P(w_n\mid w_{n-1}) + (1-\lambda)\cdot P(w_n)$$</p>

Note:
Source: Spring Lecture 02 slide 25, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/24.

---

<!-- .slide: id="interpolation-trigram" -->

## Interpolation for trigram

<p>Mix unigram + bigram + trigram (with $\sum_i \lambda_i=1$):</p>
<p>$$\begin{aligned} P_{\lambda_1,\lambda_2,\lambda_3}(w_n\mid w_{n-2}w_{n-1}) &= \lambda_1 P(w_n\mid w_{n-2}w_{n-1}) \\ &+ \lambda_2 P(w_n\mid w_{n-1}) \\ &+ \lambda_3 P(w_n) \end{aligned}$$</p>
<p class="text-bad"><strong>How to set $\lambda_i$?</strong></p>
...
<!-- .slide: id="held-out-tuning" -->

## Tune $\{\lambda_1,\lambda_2,\lambda_3\}$ on held-out data

| Training | Held-out | Test |
| :--- | :--- | :--- |
| Fix $N$-gram probabilities | Choose $\lambda_i$ | Report |

<ul>
<li>Fix $N$-gram probabilities estimated on training data.</li>
<li>Choose $\lambda_i$ to maximize held-out log-likelihood:</li>
</ul>
<p>$$\max_{\lambda_1,\lambda_2,\lambda_3}\; \sum_{i}\log P_{\lambda_1,\lambda_2,\lambda_3}(w_i\mid w_{i-2}w_{i-1})$$</p>

Note:
The EM algorithm or a simple grid search over the simplex both work for three weights. Source: Spring Lecture 02 slide 25, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/24.

---

<!-- .slide: id="katz-backoff" -->

## Method 3: Katz backoff

<img class="diagram" src="assets/Katz-backoff.png" alt="Backoff strategy: condition on less context for contexts you have not learned much about; use trigram if you have good evidence, otherwise bigram, otherwise unigram. If the N-gram has zero counts, back off to the (N-1)-gram until a history has counts. Katz backoff: P_Katz(w_n | w_{n-2} w_{n-1}) equals C-star of the trigram divided by C of the bigram history if the trigram count is positive, otherwise alpha of w_{n-1} times P_ML(w_n | w_{n-1}). C-star comes from Good-Turing; alpha is chosen so that P_Katz sums to one.">

Note:
Read the two questions at the bottom of the figure aloud: $C^{\ast}$ from Good–Turing, $\alpha$ from normalization. The figure is the Spring slide image, kept unchanged. Source: Spring Lecture 02 slide 26, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/25.

---

<!-- .slide: id="good-turing" -->

## Method 4: KN smoothing – from Good–Turing to Kneser–Ney

<p><strong>Good–Turing intuition.</strong> Pick balls from a box (colors = events). Observed counts:</p>

| 10 red | 3 blue | 2 white | <span class="text-bad">1 yellow</span> | <span class="text-bad">1 black</span> | <span class="text-bad">1 green</span> |
| :--- | :--- | :--- | :--- | :--- | :--- |

<blockquote><p>Question: how likely is the next ball a <strong class="text-token">new color</strong> (e.g., pink)?</p></blockquote>
<p><strong>Frequency-of-frequency:</strong> $N_r$ is the number of types seen exactly $r$ times.</p>

Note:
Good–Turing's answer: the probability of something new is estimated by the fraction of singletons, $N_1/N = 3/18$. Source: Spring Lecture 02 slide 27, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/26.
...
<p class="exercise-meta">Exercise E05 · 4 minutes · Notebook E05 · Show that $P^{\ast}_{\text{GT}}$ is a distribution</p>

<div class="columns columns-wide-left">
<div>
<p>Example text: <span class="text-bad"><strong>Sam I am I am Sam I do not eat</strong></span></p>
<p>Counts: I 3 · Sam 2 · am 2 · do 1 · not 1 · eat 1, so $N_0=3,\; N_1=3,\; N_2=2,\; N_3=1$.</p>
<p>$$r^{\ast} =\frac{(r+1)N_{r+1}}{N_r}, \qquad P^{\ast}_{\text{GT}}(w) =\frac{r^{\ast}}{N}$$</p>
<p>$V=\{$Sam, I, am, do, not, eat, <span class="text-bad">Bob, Ben, you</span>$\}$</p>
</div>
<div class="answer fragment"><p>Unseen mass $N_1/N = 3/10$, i.e. $0.1$ each for Bob, Ben, you. Singletons: $r^{\ast}=4/3$, so $0.133$ each. Doubletons: $r^{\ast}=3/2$, so $0.15$ each. “I”: $r^{\ast}=4N_4/N_3=0$. Total: $0.3+0.4+0.3+0=1$</p></div>
</div>

Note:
$N=10$ tokens. The total is exactly one, but the most frequent word gets zero because $N_4=0$; real implementations smooth the $N_r$ counts or stop applying Good–Turing above a threshold $k$. Source: Spring Lecture 02 slide 27, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/26.
...
<p>From Good–Turing, we often <strong>discount</strong> observed counts by a constant $d$, then redistribute the leftover mass via interpolation:</p>
<p>$$P_{\text{KN}}(w_i\mid w_{i-1}) = \frac{C(w_{i-1},w_i)-d}{C(w_{i-1})} + \lambda(w_{i-1})\cdot P(w_i)$$</p>
<ul>
<li>$\dfrac{C(w_{i-1},w_i)-d}{C(w_{i-1})}$: discounted bigram</li>
<li>$\lambda(w_{i-1})$: interpolation weight</li>
</ul>

Note:
Full Kneser–Ney also replaces the unigram $P(w_i)$ by a continuation probability, the fraction of distinct histories that $w_i$ follows; that detail is optional in the Fall course. Source: Spring Lecture 02 slide 27, https://baojian.github.io/llm-26/slides/lecture-02-slides/index.html#/26.
