...
| `held-out-bpb.svg`, `lm-filter.svg`, `self-training-loop.svg`, `lecture02-results.json` | Fall additions, generated by `scripts/lecture02_experiments.py` (standard-library SVG): bits per byte of 1/2/3-gram models on the course's held-out shards, the Wikipedia reference-model filter histogram, and the self-training loop; the JSON is the script's full output. Data: TinyStories v2 (CDLA-Sharing-1.0), OpenWebText (CC0), Fineweb-Edu-Chinese V2.1 (Apache-2.0), WikiText-103 (CC-BY-SA 3.0), Qwen3 tokenizer (Apache-2.0). |
